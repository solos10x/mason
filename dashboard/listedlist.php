...
$title = siteName.' :::  Make Listed Donations ';
$ListedOutput = '';
?>
<?php 
		$db->runQuery("SELECT * FROM incoming WHERE type = 'Listed' AND status = 'On'");
		if ($db->numRows() < 1) {
			$ListedOutput = '<div class="admin-search-action-wrap" align="center"> Please check back later for the list</div>';
			}
			else {
			...
			<td><a href="'.URL.'dashboard/listedlist?reserve='.$process->encryptDecrypt("encrypt", $activityID).'" class="btn btn-primary btn-wrap green-btn">Donate</a></td>
			</tr>';
			}													
			...
			if ($db->numRows() < 1) {
				...
				// select recipient information from user table
					...
					$output = ' <div class="portlet-body">
						   ...
						   onclick=\'controller.donateNow("'.$activityID.'", "'.$donorUID.'", "'.$donorUsername.'", "'.$recipientUID.'", "'.$recipientUsername.'", "'.$outstandingAmount.'", "'.$process->getListedSetUp("minimum").'", "'.$dollarRate.'")\'> Donate </button> </p>
                                                </div>
					...
					$output .= ' <div class="reservation-info-board">
									   ...
									   <h4> Please Note: </h4>
									  <ul> 
									  <li> Minimum donation amount for this section is  <span>$'.$process->getListedSetUp("minimum").'<span> </li>
									  <li> You gain your <span>donation + '.$process->getListedSetUp("incentive").'%</span>  within 48 hrs - 7days of donation</li>
									  <li> The system holds back <span>'.$process->getListedSetUp("hold").'%</span> of the incentive gained. This withheld amount is added up to any amount you are entitled to when you re-donate again. This is to keep the system moving efficiently </li>
                    ...
                    <div class="page-content">
                        ...
                        <div class="page-bar">
                            <ul class="page-breadcrumb">
                                ...
                                <li>
                                    <span>Make Listed Donations</span>
                                </li>
                            </ul>
